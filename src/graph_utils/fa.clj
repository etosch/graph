...
(defn tag-state [loc tag fun]
  ...
  (with-meta loc (assoc (meta loc) :nodes
			(conj (disj (graph/nodes loc) (graph/node loc))
			      (with-meta (graph/node loc) {tag (fun loc)})))))
...
(defn accepts [loc]
  (:accepts (meta (get (graph/nodes loc) (graph/node loc))) #{}))
...
(defn enumerate-strings [loc n]
  ...
  (loop [nexts (list loc) ret-val #{} steps 0]
;;    (println nexts ret-val steps)
    (if (or (empty? nexts) (> steps (Math/pow 2 (count (graph/edges loc)))))
      ret-val
      (let [this-level (graph/next-generator (first nexts))
	    accepts (map #(or (seq (graph/input-processed %)) '(:epsilon)) (filter (graph/accept-fn loc) this-level))]
	(if (> (+ (count ret-val) (count accepts)) n)
	  (reduce conj ret-val (take (- n (count accepts)) accepts))
	  (recur (concat (rest nexts) this-level)
		 (reduce conj ret-val accepts)
		 (inc steps)))))))
		 
;; (defn enumerate-strings [loc n]
;;   "Enumerates the first n strings"
;;   (loop [ret-val (transient #{}) group-1 (transient [loc]) group-2 (transient [])]
;; ;;    (println (count check-these) (count group-1) (count group-2))
;;     (if (and (zero? (count check-these)) (zero? (count group-1)) (zero? (count group-2)))
;;       ;; if my graph can't produce at least n strings, just return what I have
;;       (persistent! ret-val)
;;       (binding [buffer (graph/next-generator (nth group-1 0)) ;;get the next possible graphs at one step from now
;; 		accepts (map #(
;; 		(loop [in check-these out '()]
;; 			  ;;		      (println " " in out)
;; 			  (if (empty? in) out
;; 			(recur (rest in)
;; 			       (if ((graph/accept-fn loc) (first in))
;; 				 (cons (or (seq (graph/input-processed (first in))) :epsilon) out)
;; 				 out))))]
...
;; 		      nexts (apply list (graph/next-generator (nth group (dec (count group)))))]
;; 		     (recur (if (zero? (count accepts))
;; 			      ret-val
;; 			      (reduce conj! ret-val accepts))
;; 				      ;;(keep #(seq (graph/input-processed %)) accepts))))
;; 			    nexts
;; 			    (pop! group)
;; 			    (if (zero? group-1-size) (transient (vec nexts)) (reduce conj! group-2 nexts)))
;; 		     (persistent! (reduce conj! ret-val (take (- n (count ret-val)) accepts))))
;;       ))))
...
Verbose turns on a printer that prints to standard out one + for every 100 steps the search in the evaluation function takes."
  (let [max-visits (count (graph/input-remaining loc))
	...
	(cond (or (not me) (> (visits me) max-visits)) (and (reset! break :reject) break)
 	      ...
 	      :else (let [children (graph/next-processor (tag-state me :visits #(inc (visits %))))] ;;tag current node as I leave this state
